...
//	derived class DimNotMatchException
class DimNotMatchException : public ArrayException {
private:
	int m_size;													//	member data to store the array size
public:
	DimNotMatchException();										//	Defualt constructor
	DimNotMatchException(const DimNotMatchException& source);	//	Copy constructor
	DimNotMatchException(int size);								//	constructor with int argument
	//	Assignment operator
	DimNotMatchException& operator = (const DimNotMatchException& source);
	~DimNotMatchException();									//	Desturctor
